    ...
    ###FOr testing purposes, a model where 3 adsorption sites fitted simultaneously and 3 separately are done by comparison
        if args.multi_adsorbate == 0:
            if args.load_model == 0:
                ...
                print("Loading model...")
                model = load_model(f"models/epoch:{epoch}.h5", compile=False)
                model.compile(
                    loss="logcosh", optimizer=Adam(0.001), metrics=["mean_absolute_error"]
                )
            model.summary()
        ...
        log[f"{epoch}:train MAE"] = mean_absolute_error(y_train, train_out)
        log[f"{epoch}:train RMSE"] = mean_squared_error(y_train, train_out) ** (0.5)
        log[f"{epoch}:test MAE"] = mean_absolute_error(y_test, test_out)
        log[f"{epoch}:test RMSE"] = mean_squared_error(y_test, test_out) ** (0.5)

        with open(f"result/{data_dir}_epoch{epoch}_predict_train.txt", "w") as f:
            np.savetxt(f, np.stack((y_train, train_out), axis=-1))
        with open(f"result/{data_dir}_epoch{epoch}_predict_test.txt", "w") as f:
