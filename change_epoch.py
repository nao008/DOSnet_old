...
def main():
    ...
    with(open(f"result/epoch/{args.data_dir}_epoch_{mode}{args.kfold_num}_log.txt", "w")) as f:
...
def create_model(shared_conv, channels, seed):
    set_seed(seed)
...
def run_kfold(args, x_surface_dos_raw, x_adsorbate_dos, y_targets,log):
    ...
    for epoch in epochs:
        ...
        for seed_val in seed_list:
            ...
            epoch_log_mae.append(mean_absolute_error(test_y_CV, train_out_CV))
            epoch_log_rmse.append(mean_squared_error(test_y_CV, train_out_CV) ** (0.5))
            # ディレクトリの生成
            new_dir_path = f"result/epoch/epoch{epoch}"
            if not os.path.exists(new_dir_path):
                os.makedirs(new_dir_path)
            # 結果の書き込み
            with open(f"{new_dir_path}/{args.data_dir}_CV{args.kfold_num}_epoch_seed{seed_val}.txt", "w") as f:
        ...
        log[f"{epoch}_mae"] = np.mean(epoch_log_mae)
        log[f"{epoch}_rmse"] = np.mean(epoch_log_rmse)
