...
lr_list = [
    ...
    def decay_schedule_1(epoch, lr):
        return 0.001  # 学習率は常に0.01
    
    def decay_schedule_2(epoch, lr):
        return 0.001 / (1 + 0.1 * epoch)  # 学習率はエポック数に反比例

    def decay_schedule_3(epoch, lr):
        if epoch % 10 == 0:  # 10エポックごとに学習率を半分にする
            return lr * 0.5
        else:
            return lr
        
    def decay_schedule_4(epoch, lr):
        lr = ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=5, min_lr=0.001)  # 検証エラーが5エポック改善しない場合、学習率を0.2倍にする
        return lr
...
def run_training(args, x_surface_dos, x_adsorbate_dos, y_targets):
    ...
    for lrdef in lr_list:
        ###call and fit model
        shared_conv = dos_featurizer(args.channels)
        lr_scheduler = LearningRateScheduler(lrdef, verbose=0)
